...
class Erkek(commands.Cog):
    def __init__(self, bot):
        self.bot = bot

    @commands.slash_command(description="Kullanıcıyı erkek olarak kaydeder.")
    async def erkek(self, ctx, uye: discord.Member, isim, yas):
        ...
        erkek_rol_id = int(ayarlar["erkek_rol_id"])
        ...
        erkek_rol = discord.utils.get(ctx.guild.roles, id=erkek_rol_id)
        kayitsiz_rol = discord.utils.get(ctx.guild.roles, id=kayitsiz_rol_id)
        await uye.add_roles(erkek_rol)
        await uye.edit(nick=f"{isim} {yas}")
        await uye.remove_roles(kayitsiz_rol)

        # Veritabanına kullanıcıyı kaydet
        imlec.execute("INSERT INTO users (user_id, name, age, gender) VALUES (?, ?, ?, ?)", (uye.id, isim, yas, "Erkek"))
        baglanti.commit()
        ...
        embed.add_field(name="Rol Verildi", value=erkek_rol.mention, inline=False)
